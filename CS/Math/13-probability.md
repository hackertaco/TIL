...
## 몬테 카를로 알고리듬
1. 어떤 검사를 정해진 수만큼 수행
2. 한번 검사할 때마다 검사 대상을 무작위로 뽑음
3. 한번 검사할 때마다 둘 중 하나의 답이 나옴
  - 찾음
  - 못찾음: unknown: yes일수도 no일수도. 확실히 알려면 또 시도해야한다.
  - 정해진 수만큼 검사해도 여전히 모르겠다면? no라고 결론 내린다. 
당연히 이 주장이 틀릴 확률이 있다. 그 틀릴 확률을 엄청 작게 낮추는 것이 관건이다.

문제는, yes여야 하는 답을 찾지 못할 때다. 
틀린 답을 횟수를 늘려서 낮출 수가 있다.

### 품질 관리
잘 오류를 캐치할 확률이 10프로다.
그럼 1-0.1 = 0.9가 오판할 확률
오판할 확률을 90프로라고 할 때, 그를 2프로 확률 아래로 만들고싶다고 하면,
0.9^n <= 0.2를 계산해보면 된다.

원하는 신뢰도에 따라 검사 횟수를 변경 가능하다.

### 소수 판별
소수는 다른 수와 덜 겹친다. ex)동물의 탄생 주기

소수를 이용하여, 암호화를 할 때 보안이 더 좋을 수 있다. 대표적인 기법 중 하나는 RSA.
- 매우 큰 두 소수 2개를 뽑고
- 그 두 소수를 곱해 나온 값으로 나머지 연산을 하며 암호키를 만듦

이유는, 매우 큰 소수 둘을 곱하는 것은 빠른 연산인데, 
그 곱한 결과로부터 원래의 두 소수를 찾는 것은 엄청나게 시간이 오래 걸린다.

그런데, 매우 큰 소수가 소수인지 어떻게 알지:: 소수 판별법

주먹구구식 말고, 몬테 카를로 알고리듬을 쓸 수 있다. 

#### 밀러라빈
밀러라빈 판별법을 통과 목샇면 합성수다.
b를 하나 뽑았는데, 이 판별법을 통과못할 가능성은 3/4
그러면, 이를 합성수라는 사실을 못 맞출 확률은 1/4
(1/4)^n <= 0.000001
즉, 이를 10번 수행하면서 합성수인게 결정이 안 나면, 99.9999% 이상의 확률로 n은 소수다.

### 확률적 알고리듬과 프로그래머의 마음가짐
- 프로그램은 당연히 확률적 알고리듬을 찜찜해한다. 그래서 결정적 알고리듬을 선호한다. 
그러나 이제는 피할 수 없다.

- 확률적으로 가되, 틀릴 확률을 확 줄이면 된다.
