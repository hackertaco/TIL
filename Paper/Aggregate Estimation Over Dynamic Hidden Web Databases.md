...
- 일반적인 집계로의 확장과, 멀티 라운드
   라운드가 여러개 있을 때, 집계 쿼리나 드릴 다운하는 것은 모두 이전의 드릴 다운에 영향을 받는다.
   우리는 쿼리 함수가 있다면, 이론적으로 최고의 분산을 분석할 수 있다. 
- 알고리즘
  파일럿 드릴다운 실행 > 업데이트 예산 예측 > 랜덤하게 골라진 쿼리와 기존 드릴 다운 풀의 합집합을 새로이 드릴다운 풀로 등록
  쿼리 예산 소진할 때까지 랜덤하게 드릴 다운

## 시스템 디자인
실제 웹 데이터 베이스는 두 분야로 나뉜다. 어떻게 알고리즘을 이 두 분야에 접목할지에 대한 논의를 한다. 

- 쿼리 모델
   - 스트림 쿼리 모델: 사전 정의 쿼리 모델이라고도 불리는 이것은 다른 쿼리 처리 시작 전에 집계 쿼리를 지원한다. 
     - 계속해서 업데이트 되기 때문에, 지속적인 평가가 필요하다.
     - 업데이트 된 결과에 기반하여, 집계 예측이 이루어진다.
   - 애드혹 쿼리 모델: 데이터베이스 트래킹이 시작된 후에, 어떤 순간에나 집계 쿼리를 지원한다.
     - 한번일 수도 있고, 지속적일수도 있다. 
     - 이전의 드릴 다운에서 모든 튜플을 회수하므로, 시뮬레잍 해볼 수 있다. 

우리가 만든 알고리즘은 몇개 안되는 수정으로 쿼리 모델을 다룰 수 있다. 
그러나, 두 개의 쿼리 모델 집계 예측에 퍼포먼스 차이점이 있다는 것을 알아야한다.

- 스트림 모델
  - 모든 처리할 집계는 특정한 예측을 담고있다면, 쿼리 트리를 만들어서 모든 노드가 예측을 하게 한다. 
  - 더 작은 쿼리 트리가 더 정확하므로 이 경우는 애드혹 모델은 낄 수 없다. 
- 업데이트 모델
  - 우리의 알고리즘과 업데이트 모델을 접목하는 것은 굉장히 까다롭다. 어떻게 집계라는 것을 정의내려야할지 모른다. 
  - 그럼에도, 이 알고리즘은 파일럿 페이즈가 끝나면, 데이터 베이스의 변화를 무시하면서 가능하다.


## 실험 평가
세가지 측면에서 알고리즘을 평가했다. 1) 야후 2) 아마존 3) 이베이
Restart, Reissue, RS모두를 평가 

- 실험 결과
  - 싱글라운드
    - 세개 모두 편향되지 않은 추측을 했다. RS가 가장 적은 변동을 보였다. 
    - 에러가 시간이 지날수록 극한 상황에서 RS가 가장 줄어든다
  - 트랜스라운드
    - 큰 변화, 작은 변화 모두 테스트해본 결과 RESTART보다 다른 것들이 월등히 결과가 좋았다. 
  - 라이브 실험
    - 라이브로 실험한 결과, 시간이 지날수록 REISSUE, RS가 RESTART보다 성능이 좋았다. 
    - 더 업데이트가 잦은 경우, 더 성능이 나앗다.

## 관련 WORK
- Hidden database 정보 연동과 추출
- 집계 추측
  - 그간 많은 연구가 있었지만, 그 연구의 정보는 재사용되지는 않았다. 
  - 더불어 정적인 데이터 베이스 연구였다.
- 쿼리 프로세싱
  - 데이터베이스의 변화를 상정하지 않았고, 트랜스라운드를 진행했다는 점이 다르다

## 결론
- 쿼리 리이슈와 부트스트랩 개념의 쿼리 플랜을 제안했다. 
- 추측에러와 쿼리비용을 이론적으로 계산하였다.
- 실제 세계와 이론적 세계 접근을 위한 실험 진행을 했다.